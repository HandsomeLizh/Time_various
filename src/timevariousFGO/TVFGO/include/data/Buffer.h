...
/* 核心功能：线程安全 (Thread Safety)
在多传感器、多线程的ROS 2应用中，多个线程会同时读写数据。例如，ROS的回调线程会不断地向缓冲区写入新的传感器数据，而后端的优化线程会读取这些数据。如果不对共享的缓冲区进行保护，就会导致数据错乱或程序崩溃*/
namespace fgo::data {
  ...
  struct CircularDataBuffer {
    ...
    void clean() {
      ...
      //好处: 这种方式确保了无论函数是正常返回还是中途抛出异常，锁最终都会被释放，极大地避免了死锁问题。
      clean_();
    };
