#ifndef DATATETYPESMEASUREMENT_H
#define DATATETYPESMEASUREMENT_H
...
namespace fgo::data {
  ...
  //ToDo: @Haoming, define all lidar related data here
    ...
    struct LidarPoint {
        ...
        float intensity;
        uint16_t ring; //激光束ID
        double time; //相对于该帧起始时间的偏移
    };

  /** IMU **/
  struct IMUMeasurement {
    /*timestamp 和 dt: 时间戳和与上一帧的时间差，是积分的基础。

accLin, gyro, mag: 线性加速度、角速度和磁力计的测量值。

...Cov: 对应每个测量值的协方差矩阵。这是至关重要的信息，因为它代表了测量的不确定性，将直接用于构建GTSAM中IMU因子的噪声模型。*/
...
#endif // DATATETYPESMEASUREMENT_H